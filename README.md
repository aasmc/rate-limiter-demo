...
Казалось бы, задача простая - подкрутить настройки лимитов в reverse-proxy, который стоит перед нашим сервисом. Но есть нюанс.
Сетевая конфигурация нашего проекта очень сложная из-за требований безопасности, и перед нашим reverse-proxy стоит еще несколько
таких же. Тонкая настройка лимитов в теории возможна, но потребует серьезных усилий со стороны команды DevOps. 
В итоге было принято решение ради одного бизнесового сценария провести доработку в коде силами команды.

Как вариант можно было бы использовать Bucket4j в связке с Redis - любые in-memory
решения не подходят, так как в приложение развернуто в нескольких экземплярах. Но чтобы затащить Redis в проект опять потребовались бы
...
Решение неоптимальное, но рабочее. Учитывая нашу нагрузку, оно нам подходит. 
Теперь рассмотрим вариант с одним походом в БД.

## Вариант с одним запросом (tstzrange)
Для этого нам потребуется задействовать тип данных PostgreSQL `TSTZRANGE` - интервал `timestamp with timezone` (у нас обязательно 
...
```
На столбец `limiter_range` навешиваем исключающее ограничение, которое при попытке вставить запись будет проверять, 
...
В первом рассмотренном нами варианте есть изъян - 2 запроса в БД. Избавиться от второого запроса можно,
используя тип данных PostgreSQL `interval`, который можно использовать для прибавления к `timestamp with time zone`, и 
получить на выходе `timestamp with time zone`. Подробнее об оберациях с датами и временем в PostgreSQL можно прочесть в 
...
в нативном sql-запросе с помощью приведения типов: `duration::interval`. Также потребуется воспользоваться механизмом
...
Сам метод сервиса также становится значительно проще:
...
## Демо
В тестовом проекте подготовлен небольшое Spring Boot приложение, выставляющее наружу 2 эндпоинта:
1. Создание сущности пользователем
2. Получение списка сущностей, созданных пользователем
...
9. количество успешних проходов рейт-лимита
...
а количество успешних вызовов гораздо ниже ожидаемого. При этом в логах postgres такие записи:
...
тестировать реализацию под нагрузкой, чтобы убедиться, что вы не упустили какие-то скрытые детали.
Мы остановились на первом варианте, несмотря на то, что он требует двух походов в БД. 